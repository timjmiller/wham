...
m1 <- fit_wham(input1, do.fit = do.fit, do.osa = F, MakeADFun.silent=TRUE, retro.silent = TRUE) # turn off OSA residuals to save time
...
m2 <- fit_wham(input2, do.fit = do.fit, do.osa = F, MakeADFun.silent=TRUE, retro.silent = TRUE) # turn off OSA residuals to save time
...
m3 <- fit_wham(input3, do.fit = do.fit, do.osa = F, MakeADFun.silent=TRUE, retro.silent = TRUE) # turn off OSA residuals to save time
...
input4 <- prepare_wham_input(asap3, recruit_model=2, model_name="Ex 1: SNEMA Yellowtail Flounder",
                                    ...
                                    age_comp = "logistic-normal-miss0", basic_info = basic_info)
m4 <- fit_wham(input4, do.fit = do.fit, do.osa = F, MakeADFun.silent=TRUE, retro.silent = TRUE) # turn off OSA residuals to save time
...
plot_wham_output(mod=m4_proj, out.type='html', dir.main=tmp.dir, plot.opts = list(browse = FALSE))
