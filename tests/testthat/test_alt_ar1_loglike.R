...
mod_alt <- fit_wham(input_alt, do.fit = FALSE)
mod_dr <- fit_wham(input_dr, do.fit = FALSE)
mod_alt_dr <- fit_wham(input_alt_dr, do.fit = FALSE)

expect_equal(as.numeric(mod$fn()), as.numeric(mod_alt$fn()), tolerance=1e-6) # nll
expect_equal(as.numeric(mod$fn() - mod_dr$fn()), -1.6416, tolerance = 1e-3) # nll
expect_equal(as.numeric(mod_dr$fn()), as.numeric(mod_alt_dr$fn()), tolerance=1e-6) # nll

input$data$do_simulate_period[1] <- 0
fit <- fit_wham(input, do.osa = FALSE, do.retro = FALSE)
proj <- project_wham(fit, do.sdrep = FALSE)
