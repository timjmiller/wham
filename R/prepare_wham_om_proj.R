#' Prepare input data and parameters to add projection to a WHAM operating model
#'
#' \code{prepare_wham_om_proj} is an internal function called by \code{\link{prepare_wham_om_input}}
#' if \code{do.proj = TRUE}.
#'
#' @param om_input is output from \code{\link{prepare_wham_om_input}} with do.proj = FALSE
...
#'   }
#'
#' @return same as \code{\link{prepare_wham_om_input}}, a list ready to use with \code{\link[TMB:MakeADFun]{TMB::MakeADFun}} to simulate populations and/or data
#'   \describe{
...
#'   }
#'
#' @seealso \code{\link{prepare_wham_om_input}}, \code{\link{project_wham}}
#'
#' @export
prepare_wham_om_proj = function(om_input, proj.opts)
{
# library(wham)
...
#                 avg.yrs=NULL, cont.ecov=TRUE, use.last.ecov=FALSE, avg.ecov.yrs=NULL, proj.ecov=NULL)
  input1 <- om_input
  mod = TMB::MakeADFun(input1$data, input1$par, map = input1$map, random = input1$random, DLL = "wham", silent = TRUE)
  rep = mod$report()
  #input1 <- model$input
  data <- input1$data
  ...
  if(!is.null(proj.opts$cont.Mre)){
    if(proj.opts$cont.Mre & input1$data$M_re_model %in% c(1,3)){ # 1 = none, 3 = AR1_a (not time-varying)
    ...
    data$proj_M_opt <- ifelse(input1$data$M_re_model %in% c(2,4,5), 1, 2) # 2 = IID, 4 = AR1_y, 5 = 2D AR1
  }
  ...
  if(any(input1$data$Ecov_model > 0)){
    ...
    Ecov.proj <- matrix(rep(NA, (proj.opts$n.yrs-end.beyond)*dim(input1$par$Ecov_re)[2]), ncol=dim(input1$par$Ecov_re)[2], byrow=TRUE)
    Ecov.map <- matrix(rep(NA, (proj.opts$n.yrs-end.beyond)*dim(input1$par$Ecov_re)[2]), ncol=dim(input1$par$Ecov_re)[2], byrow=TRUE)
  ...
  #tmp <- matrix(as.integer(map$log_NAA), data$n_years_model, data$n_ages)
  #tmp <- rbind(tmp, matrix(NA, nrow=proj.opts$n.yrs, ncol=data$n_ages))
  tmp <- par$log_NAA
  if(data$n_NAA_sigma < 2) tmp[,-1] <- NA # don't estimate NAA_devs for ages > 1 if SCAA or RE on recruitment
  if(data$n_NAA_sigma == 0) tmp[data$n_years_model+1:proj.opts$n.yrs-1,1] <- NA # for SCAA, don't estimate fixed effect rec devs in projection years
    ...
    if(end.beyond < proj.opts$n.yrs){ # need to pad Ecov_re
      for(i in 1:(proj.opts$n.yrs-end.beyond)){
        if(proj.opts$use.last.ecov){ # use last Ecov (pad Ecov_re but map to NA)
          Ecov.proj[i,] <- rep$Ecov_re[data$ind_Ecov_out_end+1+end.beyond,]
        }
        if(!is.null(proj.opts$avg.ecov.yrs)){ # use average Ecov (pad Ecov_re but map to NA)
          ...
          Ecov.proj[i,] <- avg_cols(as.matrix(rep$Ecov_re[avg.yrs.ind.ecov,]))
        }
