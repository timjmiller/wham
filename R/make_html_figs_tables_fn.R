make_html_figs_tables_fn <- function(od, do.html = TRUE, do.tex = FALSE) {
  ...
  if(do.html) rmarkdown::render(file.path(od,"figures_tables.Rmd"), output_format = "html_document", output_file = file.path(od, "wham_figures_tables.html"), 
  ...
  if(do.tex) { #for some reason on windows working outside of the temp directory was causing issues for tinytex::latexmf.
    origdir = getwd()
    setwd(od)
    try(rmarkdown::render("figures_tables.Rmd", output_format = "pdf_document", output_file = "wham_figures_tables.pdf", quiet = T))
    setwd(origdir)
  }
