set_osa_obs = function(input)
{
      ...
      if(data$use_index_caal[y,i]==1) 
      {
        for(l in 1:data$n_lengths){
          ...
          if(length(ind)) {
            ...
            data$use_index_caal[y,i] <- 0 #set to not use because there are not enough positive values 
      ...
      if(data$use_catch_caal[y,i]==1) 
      {
        for(l in 1:data$n_lengths){
          ...
          if(length(ind)) {
            ...
            data$use_catch_caal[y,i] <- 0 #set to not use because there are not enough positive values 
  ...
  data$do_post_samp = rep(0,8) #this will be changed in fit_wham when a sample of posterior process residuals are to be calculated
  #length = 5, whether to ADREPORT posterior residuals for NAA, M, selectivity, Ecov, q, growth (or LAA), LW parameters, WAA
