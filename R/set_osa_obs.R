...
set_osa_obs <- function(input)
{
  ...
  if(!is.null(input$by_pwi) | !is.null(input$options$selectivity)) { #only do if called by prepare_wham_input or if set_selectivity has been called with options
  ...
  } else { #is.null(input$by_pwi) & is.null(input$options$selectivity)
    ages_omit <- NULL
    input$log$osa_obs <- c(input$log$osa_obs, paste0("set_osa_obs has been called outside of prepare_wham_input and set_selectivity has not yet been called with options.","\n",
      "So osa_obs for age comp may not yet be correctly specified. Call set_selectivity with selectivity options again before trying to create or fit a model.\n\n"))
  }
  ...
  for(y in 1:data$n_years_model){
    ...
    for(i in 1:data$n_indices){
      ...
      # print(dim(data$selblock_pointer_indices))
      # print(data$age_comp_model_indices)
      # print(dim(x))
      if(data$use_index_paa[y,i] == 1)
      {
        obs_y <- x[y,]
        tmp <- integer(0)
        if(!is.null(ages_omit)) tmp <- ages_omit[[data$selblock_pointer_indices[y,i]]]
    ...
    for(i in 1:data$n_fleets){
      ...
      if(data$use_catch_paa[y,i] == 1) {
        obs_y <- x[y,]
        tmp <- integer(0)
        if(!is.null(ages_omit)) tmp <- ages_omit[[data$selblock_pointer_fleets[y,i]]]
        #multinom, D-M, mvtweedie
  ...
  if(length(input$log$osa_obs))  input$log$osa_obs <- c(
    "--OSA obs----------------------------------------------------------------------------------------------------------------------------",
    "\n", 
    input$log$osa_obs,
    "-------------------------------------------------------------------------------------------------------------------------------------",
  "\n\n")
  if(is.null(input$by_pwi)) message(unlist(input$log$osa_obs, recursive=T))
  return(input)
}
