verify_version = function(model){
  ...
  wham_commit <- strsplit(model$wham_version, " / ")[[2]][1]
  wham_commit <- substr(desc$wham_commit,1,7)
  if(wham_version !=  model$wham_version) {
    ...
    "devtools::install_github('timjmiller/wham', dependencies=TRUE, ref=", wham_commit, ") \n"))
  }
  ...
  TMB_commit <- strsplit(model$TMB_version, " / ")[[2]][1]
  TMB_commit <- substr(TMB_commit,1,7)
  if(TMB_version !=  model$TMB_version) {
    stop(paste0("your loaded TMB version: \n ", TMB_version, 
    " \n is not the same as that used to fit your model: \n", 
    model$TMB_version, " \n Load the correct version. If necessary install the right version using \n",
    "devtools::install_github('kaskr/adcomp/TMB', dependencies=TRUE, ref=", TMB_commit, ") \n"))
  }
}
