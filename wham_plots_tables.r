...
not.load = which(!sapply(pkgs, function(x) library(x, logical.return = TRUE, character.only = TRUE)))
if(length(not.load)) sapply(not.load, function(x) 
{
...
plot.FXSPR.annual <- function(mod, alpha = 0.05, status.years, max.x, max.y)
{
  ...
  for(i in 1:3)
  {
    ...
    ci <-  vals + cbind(qnorm(1-alpha/2)*cv, -qnorm(1-alpha/2)*cv)
	  plot(years, exp(vals), xlab = '', ylab = t.ylab, ylim = c(0,max(exp(ci))), type = 'l', cex.lab = 2)
	  grid(col = gray(0.7))
