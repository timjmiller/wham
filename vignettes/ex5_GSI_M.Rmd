...
This is the 5th WHAM example, which builds off [example 2](https://timjmiller.github.io/wham/articles/ex2_CPI_recruitment_SNEMA_yellowtail.html):
...
We assume you already have `wham` installed. If not, see the [Introduction](https://timjmiller.github.io/wham/). The simpler 1st example, without environmental effects or time-varying $M$, is available as a [R script](https://github.com/timjmiller/wham/blob/master/inst/example_scripts/ex1_SNEMA_yellowtail_flounder.R) and [vignette](https://timjmiller.github.io/wham/articles/ex1_SNEMA_yellowtail_flounder.html).
...
We need the same ASAP data file as in [example 2](https://timjmiller.github.io/wham/articles/ex2_CPI_recruitment_SNEMA_yellowtail.html), and the environmental covariate (GSI). Let's copy `ex2_SNEMAYT.dat` and `GSI.csv` to our analysis directory:
```{r eval=FALSE}
wham.dir <- find.package("wham")
...
As described in [example 2](https://timjmiller.github.io/wham/articles/ex2_CPI_recruitment_SNEMA_yellowtail.html), the environmental covariate options are fed to `prepare_wham_input()` as a list, `ecov`. This example differs from example 2 in that:
...
Models `m8` (estimate mean $M$ and 2D AR1 deviations by year and age, no GSI effect) and `m17` (keep mean $M$ fixed but estimate 2D AR1 deviations) had the lowest AIC and were overwhelmingly supported relative to the other models (**bold** in table below).
...
thebest = which(vign5_res$dAIC < 2) # m8 and m17
vign5_res %>%
...
Model `m17` had $\Delta AIC = 1.5$ and the lowest retrospective pattern for all numbers-at-age and F. Model `m17` left $M_a$ fixed at the values from the ASAP data file (as in `m1`) and estimated 2D AR1 deviations around these mean $M_a$. This is how $M$ was modeled in [Cadigan (2016)](https://www.nrcresearchpress.com/doi/10.1139/cjfas-2015-0047).
...
![Kobe status plot, m1.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex5_plots/m1_Kobe_status.png){ width=31% }![Kobe status plot, m8.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex5_plots/m8_Kobe_status.png){ width=31% }![Kobe status plot, m17.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex5_plots/m17_Kobe_status.png){ width=31% }
