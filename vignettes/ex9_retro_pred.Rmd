...
For each model, peel 3-15 years of data and re-fit the model using `retro()`, and then project each peel 3 years by calling `project_wham()`. These projections are done with *F* = 0 and continuing the CPI AR1 process model (without new observations). You can specify how to run the projections using `$proj.opts` (see [`project_wham()`](https://timjmiller.github.io/wham/reference/project_wham.html) and [Ex 3: Projections](https://timjmiller.github.io/wham/articles/ex3_projections.html)).
