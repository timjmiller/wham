...
As in [example 5](https://timjmiller.github.io/wham/articles/ex5_GSI_M.html), the GSI data file does not have a standard error estimate, either for each yearly observation or one overall value. In such a case, WHAM can estimate the observation error for the environmental covariate, either as one overall value, $\sigma_{\mathrm{GSI}}$, or yearly values as random effects, $\mathrm{log}\sigma_{\mathrm{GSI}_y} \sim \mathcal{N}(\mathrm{log}\sigma_{\mathrm{GSI}}, \sigma^2_{\sigma_{\mathrm{GSI}}})$. In this example we choose the simpler option and estimate one observation error parameter, shared across years.
...
- `ecov$logsigma = "est_1"` estimates the GSI observation error ($\sigma_{\mathrm{GSI}}$, one overall value for all years like in example 5). The other option is `"est_re"` to allow the GSI observation error to have yearly fluctuations (random effects). The Cold Pool Index in example 2 had yearly observation errors given, so this was not necessary.
- `ecov$R_how = matrix("none",1,1)` or `ecov$R_how = NULL` estimates the GSI time-series model (AR1) for models without a GSI-Recruitment effect, in order to compare AIC with models that do include the effect. Setting `ecov$R_how = matrix("limiting-lag-1-linear,1,1)` specifies that the GSI in year $t$ affects the Beverton-Holt $\beta$ parameter ("limiting" / carrying capacity effect) in year $t+1$ linearly (on log scale).
...
vign6_res[1,"AIC"] = '---'
vign6_res |>
  dplyr::rename("Converged"="conv", "PD Hessian"="pdHess", "Runtime (min)"="runtime", "$\\Delta AIC$"="dAIC",
...
![NAA_devs.png](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/NAA_devs.png){ width=100% }
...
![m1 NAA_retro_relative](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m1_NAA_retro_relative.png){ width=50% }
![m13 NAA_retro_relative](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m13_NAA_retro_relative.png){ width=50% }

###### Spawning stock biomass, SSB

![m1 SSB_retro_relative](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m1_SSB_retro_relative.png){ width=50% }
![m13 SSB_retro_relative](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m13_SSB_retro_relative.png){ width=50% }

###### Fishing mortality, F

![m1 Fbar_retro_relative](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m1_Fbar_retro_relative.png){ width=50% }
![m13 Fbar_retro_relative](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m13_Fbar_retro_relative.png){ width=50% }
...
![Gulf Stream Index (GSI) time-series model, m7.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m7_Ecov_1.png){ width=50% }
![Gulf Stream Index (GSI) time-series model, m13.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m13_Ecov_1.png){ width=50% }
...
![m1_SSB_F_trend](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m1_SSB_F_trend.png){ width=50% }
![m7_SSB_F_trend](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m7_SSB_F_trend.png){ width=50% }
...
![Kobe status plot, m7.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m7_Kobe_status.png){ width=50% }
![Kobe status plot, m13.](https://raw.githubusercontent.com/timjmiller/wham/master/vignettes/ex6_plots/m13_Kobe_status.png){ width=50% }
