...
We need the same ASAP data file as in [example 1](https://timjmiller.github.io/wham/articles/ex1_basics.html), and the environmental covariate (Gulf Stream Index, GSI). Let's copy `ex1_SNEMAYT.dat` and `GSI.csv` to our analysis directory:
```{r eval=FALSE}
wham.dir <- find.package("wham")
...
for(m in 1:n.mods){
  ...
  ecov <- list(
    ...
    where = where = c("none","recruit")[as.logical(df.mods$GSI_how[m])+1], # GSI affects recruitment
...
All models except for `m4` converged and successfully inverted the Hessian to produce SE estimates for (fixed effect) parameters. WHAM stores this information in `mod$na_sdrep` (should be `FALSE`), `mod$sdrep$pdHess` (should be `TRUE`), and `mod$opt$convergence` (should be `0`). See `stats::nlminb()` and `TMB::sdreport()` for details.
...
The base model, `m1`, had a severe retrospective pattern for recruitment (very high $\rho_R$). The full state-space model effectively alleviated this, whether or not a GSI-Recruitment effect was included ($\rho_R$ for `m5-7` and `m10-13` all below 0.4).  Adding a GSI-Recruitment link to the state-space models further reduced $\rho_R$, and also reduced $\rho_{SSB}$ and $\rho_{\overline{F}}$ (although $|\rho_{SSB}|$ and $|\rho_{\overline{F}}|$ were negligible for `m5-7`, < 0.05). 
