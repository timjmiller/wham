...
Here we assume you already have `wham` installed. If not, see the [README](https://github.com/timjmiller/wham#installation-and-basic-use). This is the 3rd `wham` example, which builds off model `m6` from [example 2](https://github.com/timjmiller/wham/blob/master/vignettes/ex1_SNEMA_yellowtail_flounder.Rmd):
...
We need the same data files as in [example 2](https://github.com/timjmiller/wham/blob/master/vignettes/ex1_SNEMA_yellowtail_flounder.Rmd). Let's copy `ex2_SNEMAYT.dat` and `CPI.csv` to our analysis directory:
```{r eval=FALSE}
wham.dir <- find.package("wham")
...
Setup model `m6` from [example 2](https://github.com/timjmiller/wham/blob/master/vignettes/ex1_SNEMA_yellowtail_flounder.Rmd):
...
2. Add projections with initial model fit (default `do.proj = TRUE`)

```{r eval=FALSE}
# don't run
mod_proj <- fit_wham(input)
```
