...
fit.wham.fn = function(input, n.newton = 3, do.sdrep = TRUE, do.retro = TRUE, n.peels = 7)
{
  mod <- MakeADFun(input$data,input$par,DLL="wham_v1", random = input$random, map = input$map)
...
peel.fit.fn = function(peel, model, do.sdrep = FALSE, version = "wham_v1", n.newton = 3)
{
  out = list()
  print(peel)
  temp = model
  ...
  temp.mod <- MakeADFun(temp$dat,temp$par,DLL=version, random = temp$random, map = temp$map)
