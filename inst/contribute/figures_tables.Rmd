...
for(s in 1:dat$n_stocks) for(r in 1:dat$n_regions){
  ...
  out = round(out, 0)
  out <- out %>% kable(format = type, booktabs = T, escape=F, longtable = T, row.names = T, label = paste0(stock.names.l[s], "-", region.names.l[r], "-NAA-table"), 
...
for(r in 1:dat$n_regions){
  ...
  out = round(out, 3)
  out <- out %>% kable(format = type, booktabs = T, escape=F, longtable = T, row.names = T, label = paste0(region.names.l[r], "-FAA-table"),
...
for(r in 1:dat$n_fleets){
  ...
  out = round(out, 3)
  out <- out %>% kable(format = type, booktabs = T, escape=F, longtable = T, row.names = T, label = paste0(fleet.names.l[r], "-FAA-fleet-table"),
