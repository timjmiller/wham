...
# devtools::load_all(path = here::here()) # if you open R from wham directory
library(ggplot2)
library(tidyr)
library(dplyr)
...
# save plot with original recruitment time series only
png(file.path(write.dir,"om_rec.png"), width=11, height=8, res=300, units='in')
...
# plot with updated recruitment time series
png(file.path(write.dir,"om_updated_rec.png"), width=11, height=8, res=300, units='in')
plot(om$years_full, log(om$rep$NAA[,1]), type = 'l', xlab = "Year", ylab = "log(Recruits (1000s))")
lines(om$years_full, log(temp$rep$NAA[,1]), col = "red")
