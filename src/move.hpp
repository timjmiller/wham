  ...
  if((mu_model != 1) & (mu_model != 5)){ //some type of random effects
    ...
    if((mu_model == 2) | (mu_model == 6)){ // age random effects
      for(int s = 0; s < ns; s++) for(int t = 0, t < n_seasons; t++) for(int r = 0; r < n_regions; r++){
        ...
        Type rho_mu_a = inv_logit(mu_repars(s,0,t,r,1),-1,1,1);//rho_trans(mu_repars(s,0,t,r,1));
    ...
    if((mu_model == 3) | (mu_model == 7)){ // year random effects
      for(int s = 0; s < ns; s++) for(int t = 0, t < n_seasons; t++) for(int r = 0; r < n_regions; r++) {
        Type sigma_mu = exp(mu_repars(s,0,t,r,0));
        Type rho_mu_y = inv_logit(mu_repars(s,0,t,r,12),-1,1,1);// rho_trans(mu_repars(s,0,t,r,2));
    ...
    if((mu_model == 4) | (mu_model == 8)){ // age and year random effects
      for(int s = 0; s < ns; s++) for(int t = 0, t < n_seasons; t++) for(int r = 0; r < n_regions; r++) {
        Type sigma_mu = exp(mu_repars(s,0,t,r,0));
        Type rho_mu_a = inv_logit(mu_repars(s,0,t,r,1),-1,1,1);//rho_trans(mu_repars(s,0,t,r,1));
        Type rho_mu_y = inv_logit(mu_repars(s,0,t,r,2),-1,1,1);//rho_trans(mu_repars(s,0,t,r,2));
        Type Sigma_MU = pow(pow(sigma_mu,2) / (1-pow(rho_mu_y,2)),0.5); //marginal sd
...
template <class Type>
additive_ln_transform(vector<Type> x, int region, vector<int> can_move, int must_move){
  /* 
  ...
  */
  ...
  if(mig_type(stock) == 0) //migration is instantaneous after survival and mortality, so P is easy.
  {
    for(int r = 0; r < n_regions; r++) {
      ...
      for(int j = 0; j < n_regions; j++) can_move_r(j) = can_move(s,a,t,r,j);
      vector<Type> pmove = additive_transform(trans_par, r, can_move_r, must_move(s,a,t,r));
