...
In this vignette we show how to create input objects with
...
This exercise is peformed for 1 and 2 stocks.
...
2.  Create inputs for single stock model
    ...
    - Load asap file
    - create arguments to `prepare_wham_input` and functions specific to
      different model components (e.g., `set_M`)
    - create input by passing all arguments to `prepare_wham_input`
    - create input by passing arguments to specific functions
      sequentially
...
For a clean, runnable `.R` script, look at `ex13_no_ASAP.R` in the
...
We will extract all of the elements of the `input` needed to create a
wham input object without using to the `asap3` object in
`prepare_wham_input`. To adapt this for an analysis without an asap dat
file, we only need to supply each of the element to prepare_wham_input
structured correctly.

There is a `basic_info` argument to `prepare_wham_input` that provides a
list of various “basic” model structure information:
...
There is a `catch_info` argument to `prepare_wham_input` that provides a
list of various catch data and fleet information:
...
Similarly, there is an `index_info` argument to `prepare_wham_input`
that provides a list of various index data and information:
...
Other arguments to `prepare_wham_input` have elements that deal with
parameters of the model that may or may not be estimated and may
configure process errors. First, the `selectivity` argument (see
...
Then the `M` argument (see
[`?set_M`](https://timjmiller.github.io/wham/reference/set_M.md)). Here
we are specifying the initial values to use for annual natural mortalty
at age (by stock and region), but, by default it will not be estimated:
...
The `F` argument (see
[`?set_F`](https://timjmiller.github.io/wham/reference/set_F.md)). Here
we are specifying the initial values to use for the annual
fully-selected fishing mortality parameters for each fleet:
...
The `q` argument (see
[`?set_q`](https://timjmiller.github.io/wham/reference/set_q.md)). Here
we are specifying the initial values to use for the catchability of the
indices:
...
Finally the `NAA_re` argument (see
...
The functions for each component of the input allow the user to quickly
change just one aspect of the model and leave everything else the same.
...
First, let’s create the wham models without fitting them so the
objective function is evaluated at the intial parameters. Note there are
no process errors/random effects specified in these inputs.
...
Now evaluating the objective function (by default at the initial
values), shows that the log-likelihoods are the same for each of the
models from inputs created different ways:
...
Also if we fit the models, we see that the maximized log-likelhoods are
also identical:
...
Read in the ASAP3 .dat files into R and create an input from it. WHAM
will by default specify each ASAP dat file to represent a different
stock in a separate region without movement:
...
As for the single stock example, we will extract all of the elements of
the `input` needed to create a wham input object without using to the
`asap3` object in `prepare_wham_input`.

The various list arguments to prepare_wham_input or model component
functions are filled out with 2 stock and 2 region structure:
