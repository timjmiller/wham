...
This is the 11th WHAM example. We assume you already have `wham`
installed and are relatively familiar with the package. If not, read the
...
- Different effects of the same environmental covariate on catchability
  and recruitment.
...
Make a lists of information to pass to the `basic_info` , `catch_info`,
`index_info` and `F` arguments of `prepare_wham_input`. These components
will define a simple default stock and observations on it. We’ll then
use the input with `fit_wham` to create operating and estimating models.
This similar to example 10, but now there are two indices.
...
Here we specify recruitment deviations are independent random effects
and no stock-recruit relationship will be assumed.
...
We can use the `initial_q` and `prior_sd` components of the
`catchability` argument to specify a normal prior distribution for logit
catchability of one or more indices. The lower and upper bounds of each
catchability can be defined using `q_lower` and `q_upper` elements of
the `cactchability` argument, but they are 0 and 1000 by default.
Entries in `prior_sd` that are `NA` do not have time-varying random
effects values and the corresponding value `initial_q` will be the
initial value when estimated or the assumed catchability for simulation
for an operating model. Non-NA entries in `prior_sd` define the standard
deviation of the normal prior distribution and the corresponding value
of `initial_q` is the inverse-logit of the mean of the prior
distribution.
...
We can then define an operating model (OM) and simulate data as well as
random effects for the q of the second survey and recruitment from this
input. Note we first remove any parameters from the random component of
the input so that TMB will not try to optimize these random effects if
it would normallly when estimating a model. This is not necessary here,
but it was for the closed-loop simulations in example 10, and would seem
to be a good habit.
...
Now put the simulated data in an input file with all the same
configuration as the operating model.
...
Fit an estimation model that is the same as the operating model
(self-test).
...
This plot of the q prior and approximate posterior is provided by
`plot_wham_output`. The true simulated q is shown as the solid vertical
line.
...
![prior_posterior_q](https://raw.githubusercontent.com/timjmiller/wham/devel/vignettes/ex11_plots/prior_posterior_q.png)  
...
This is similar to what is provided in `plot_wham_output`, but with the
true values from the simulation also plotted
...
![q_time_series_1](https://raw.githubusercontent.com/timjmiller/wham/devel/vignettes/ex11_plots/q_time_series_1.png)  

## 6. Add Environmental covariates to the model

First simulate and estimate environmental covariate processes, but no
...
We use `process_mean_vals`, `process_sig_vals` and `process_cor_vals` to
specify parameters for the AR1 processes assumed for the environmental
covariate processes. Note that `ecov$q_how` is a matrix with rows for
indices and columns for covariates for whether each covariate affects an
index. Note also, that the simulated values for the observations for
each covariate here will not be used.

We will not include a prior distribution for the second index, but we
will keep the iid random effects on q for the first index.
...
Check the table of parameter estimates on the second tab of the html
document produced by `plot_wham_output`. It shows parameter estiamtes,
standard errors, and confidence intervals on the natural scale of the
parameters.

Again, this is similar to what is provided in `plot_wham_output`, but
with the true values from the simulation also plotted.
...
![q_time_series_2](https://raw.githubusercontent.com/timjmiller/wham/devel/vignettes/ex11_plots/q_time_series_2.png)  

Let’s take a look at the true and estimated environmental covariate
process pararameters. Columns are the different covariates. Rows are the
mean, sd, and correlation parameters, respectively.
...
Compare true and estimated (log) standard deviation of time-varying q.
...
The estimated variability in q is lower than truth, but the estimate has
...
We set the value for effect of the environmental covariate on q using
`ecov$beta_q_vals` (array dims are n_indices, n_Ecov, order of the
polynomial).
...
As above, create the operating model, simulate and fit data and compare
...
![q_time_series_3](https://raw.githubusercontent.com/timjmiller/wham/devel/vignettes/ex11_plots/q_time_series_3.png)  

## 8. Different effects on q and recruitment of the same environmental covariate.

Add covariates to the model and allow effects of first covariate on q
for second index AND recruitment. Then proceed as above. Use
ecov\$beta_R_vals to specify effect size of covariate on recruitment.
...
Create the operating model, simulate and fit data, and compare true and
...
![q_time_series_4](https://raw.githubusercontent.com/timjmiller/wham/devel/vignettes/ex11_plots/q_time_series_4.png)  
