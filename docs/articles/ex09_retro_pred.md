...
This is the 9th WHAM example. We assume you already have `wham`
...
In this vignette we show how to calculate retrospective predictions, by
peeling *x* years of data, re-fitting models, and then projecting each
peel *y* years. Although this can be done for any variable, we
demonstrate how to do so for recruitment, as in Fig. 4 of [Xu et
al. (2018)](https://doi.org/10.1111/fog.12236). We use the same data and
model setup as for `m3` and `m5` from [example
...
2](https://timjmiller.github.io/wham/articles/ex2_CPI_recruitment.html).
Let’s copy `ex2_SNEMAYT.dat` and `CPI.csv` to our analysis directory:
...
Setup models `m5` (with CPI effect on recruitment) and `m3` (no CPI
effect on recruitment) from [example
2](https://timjmiller.github.io/wham/articles/ex2_CPI_recruitment.html):
...
- CPI has a “limiting” (e.g. carrying capacity, [Iles and Beverton
...
For each model, peel 3-15 years of data and re-fit the model using
[`retro()`](https://timjmiller.github.io/wham/reference/retro.md), and
then project each peel 3 years by calling
[`project_wham()`](https://timjmiller.github.io/wham/reference/project_wham.md).
These projections are done with *F* = 0 and continuing the CPI AR1
process model (without new observations). You can specify alternative
options for the projections using `$proj.opts` (see
[`project_wham()`](https://timjmiller.github.io/wham/reference/project_wham.html)
and [Ex 3:
Projections](https://timjmiller.github.io/wham/articles/ex3_projections.html)).
...
Here is a function to extract and plot the retrospective predictions of
recruitment from each model. It can easily be modified to extract other
variables of interest, e.g. SSB or F.
...
The two black lines represent the “true” recruitment from each model,
...
projected recruitment (3 years).

![retro_pred_R](https://raw.githubusercontent.com/timjmiller/wham/devel/vignettes/ex9_plots/retro_pred_R.png)  
